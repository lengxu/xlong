...
object FlinkNetworkModelBenchmark {
  ...
  def main(args: Array[String]) {
    ...
    val checkpoint10s = startBenchmark("checkpoint=10s", FlinkBenchmarkConfig(checkPointInterval = TimeUnit.MINUTES.toMillis(1)))
    val checkPoint30s = startBenchmark("checkpoint=30s", FlinkBenchmarkConfig(checkPointInterval = TimeUnit.MINUTES.toMillis(1)))
    val checkpoint60s = startBenchmark("checkpoint=60s", FlinkBenchmarkConfig(checkPointInterval = TimeUnit.MINUTES.toMillis(1)))
